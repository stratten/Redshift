# New file: `RedShiftMobile/RedShiftMobile/Views/VisualizerBarsView.swift`

Reusable bars view shared by `MiniPlayerView` (small) and `NowPlayingView` (large) — see `05a-edit-MiniPlayerView.md` and `05b-edit-NowPlayingView.md` for how each embeds it.
...
Placement/sizing (applied via the edits in `05a`/`05b`):
- `MiniPlayerView`: `barWidth: 2.5, spacing: 2, minHeight: 3, maxHeight: 14` — small, sits inline next to track info.
- `NowPlayingView`: `barWidth: 6, spacing: 5, minHeight: 6, maxHeight: 32` — larger, centered under the track info block.
